...
public class SandboxListas
{
    ...
    {
        int[] copiaArreglo = new int[listaEnteros.size()];
        for(int i = 0; i < copiaArreglo.length ; i++) {
        	copiaArreglo[i] = listaEnteros.get(i);
        }
        return copiaArreglo;
    }
    ...
    public void volverPositivos( )
    {
    	...
    	for(int entero : listaEnteros) {
    		if(entero < 0 ) {
    			listaTemp.add(entero * -1);
    		}else {
    			listaTemp.add(entero);
    		}
    	}
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (int i = 0; i < listaEnteros.size(); i++) {
            ...
            if (yaContados.contains(valorActual) && !valoresRepetidos.contains(valorActual)) {
                valoresRepetidos.add(valorActual);
            }else {            	
            	yaContados.add(valorActual);
            }
        }
